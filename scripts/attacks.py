...
# Stats de ImageNet usadas en tu pipeline
IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406]).view(1,3,1,1)
IMAGENET_STD  = torch.tensor([0.229, 0.224, 0.225]).view(1,3,1,1)

def _pixel_eps_to_normalized(eps_px: float, device: torch.device):
    """Convierte ε en píxeles (0..255) a ε por canal en espacio normalizado."""
    eps = (eps_px / 255.0) / IMAGENET_STD.to(device)  # shape (1,3,1,1)
    return eps

def _clamp01_from_normalized(x_norm: torch.Tensor):
    """Convierte de normalizado a [0,1] para clamp y vuelve a normalizar."""
...
def fgsm(model, x_norm: torch.Tensor, y: torch.Tensor, eps_px: float, targeted: bool=False):
    """FGSM en espacio normalizado. eps_px en píxeles (0..255)."""
...
def bim(model, x_norm: torch.Tensor, y: torch.Tensor, eps_px: float, alpha_px: float, steps: int, targeted: bool=False):
    """Iterative FGSM (BIM). alpha_px y eps_px en píxeles."""
    device = x_norm.device
    eps = _pixel_eps_to_normalized(eps_px, device)
    ...
    for _ in range(steps):
        ...
        grad_sign = x.grad.detach().sign()
        x = x + alpha * grad_sign
        # Proyecta al L_inf-ball alrededor de x0
...
def pgd(model, x_norm: torch.Tensor, y: torch.Tensor, eps_px: float, alpha_px: float, steps: int, rand_init: bool=True, targeted: bool=False):
    """PGD L_inf en espacio normalizado."""
    device = x_norm.device
    eps = _pixel_eps_to_normalized(eps_px, device)
    alpha = _pixel_eps_to_normalized(alpha_px, device)
    x0 = x_norm.clone().detach()
